 ...
 * Uses nodemailer with Ethereal for development, configurable SMTP for production.
 * Enhanced for EP-248 with signature confirmation emails.
 *
...
export class EmailService {
   ...
   * Falls back to console logging if nodemailer is not installed.
   */
  static async getTransporter(): Promise<any> {
    if (EmailService.transporter) {
      return EmailService.transporter;
    }

    if (!nodemailer) {
      console.warn('nodemailer not installed. Emails will be logged to console. Run: npm install nodemailer');
      return null;
    }
   ...
   * Uses nodemailer with Ethereal in dev, SMTP in production.
   * Falls back to console logging if nodemailer is unavailable.
   */
  static async send(to: string, subject: string, body: string): Promise<{ success: boolean; messageId?: string; previewUrl?: string | false; error?: string }> {
    try {
      ...
      const info = await transporter.sendMail({
        from: process.env.EMAIL_FROM || '"eDocs" <noreply@edocs.dev>',
        to,
        subject,
        html: body,
      });
  ...
  /**
   * Send a signature confirmation email with signed document info.
   * EP-248: Signature confirmation and email delivery.
   */
  ...
  /**
   * Send signed document email with download links for the signed PDF and certificate.
   * EP-251: Signed document + certificate delivery.
   */
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center" style="padding: 8px 0;">
                    <a href="${signedPdfUrl}"
                       style="display: inline-block; background-color: #1a56db; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 6px; font-size: 15px; font-weight: 600; margin-right: 12px;">
